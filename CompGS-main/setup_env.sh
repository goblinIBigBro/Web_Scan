...
ENV_NAME="${COMPGS_ENV_NAME:-CompGS_env}"
...
"${CONDA_FRONTEND}" install -y -c pytorch -c nvidia -c conda-forge \
  ...
  numpy=1.26.* pillow=10.* plyfile=1.1.* pyyaml tqdm tensorboard prettytable
...
if [ ! -d submodules/diff-gaussian-rasterization ] || [ ! -d submodules/knn_dist ]; then
  echo "Missing CompGS CUDA submodules." >&2
  echo "Restore submodules/diff-gaussian-rasterization and submodules/knn_dist before running this script." >&2
  echo "Expected CUDA_HOME=${CUDA_HOME} and TORCH_CUDA_ARCH_LIST=${TORCH_CUDA_ARCH_LIST} for RTX 4090 builds." >&2
  exit 1
fi

python -m pip install --no-build-isolation -v submodules/diff-gaussian-rasterization
python -m pip install --no-build-isolation -v submodules/knn_dist
