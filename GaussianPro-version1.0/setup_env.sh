...
ENV_NAME="${GAUSSIANPRO_ENV_NAME:-gaussianpro}"
...
python -m pip install lpips opencv-python matplotlib imageio
...
require_setup_py() {
  ...
  if [ ! -f "${module_dir}/setup.py" ]; then
    ...
    echo "Restore GaussianPro submodules before running this script:" >&2
    echo "  git submodule update --init --recursive" >&2
    echo "If the repository was uploaded without submodules, restore diff-gaussian-rasterization and simple-knn from the official 3DGS sources." >&2
    exit 1
  fi
}
...
require_setup_py "${ROOT_DIR}/submodules/Propagation"
...
python -m pip install --no-build-isolation -v "${ROOT_DIR}/submodules/Propagation"
